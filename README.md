...
- **AI Suggested** keywords appear as teal editable rows below the proximity suggestions. Up to 3 keyword phrases are generated by the llava VLM running locally via Ollama, based on what the model sees in the clip's thumbnail frame. The section is hidden if Ollama is not running.
